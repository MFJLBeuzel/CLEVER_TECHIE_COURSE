...
function scrape_imdb($year_start, $year_end, $page_start, $page_end)
{
    ...
    for ($page = $page_start; $page <= $page_end; $page++) {
        $url = "http://www.imdb.com/search/title?year=$year_start,$year_end&title_type=
        feature&sort=moviemeter,asc&page=$page&ref_=adv_nxt";
...
//match ratings individually
        ...
        <span class="metascore  (favorable|mixed|unfavorable)">(.*?)\s*?<\/span>
        \s*?Metascore\s*?<\/div>\s*?<\/div>\n)?<p class="text-muted">(.*?)<\/p>!is',
        $result, $match);

        for ($i=0;$i<count($match[0]);$i++) {
            if (preg_match('!metascore  (favorable|mixed|unfavorable)">(.*?)\s*?
            </span>!', $match[0][$i], $metascore)) {
...
preg_match_all('!<p class="">(.*?)<\/p>!is', $result, $match);
        ...
        $clean_directors = preg_replace('!(<a href="\/name\/.*?\/?ref_=adv_li_dr_
        \d"\n>|<\/a>|\n)!', '', $directors[1]);
...
//votes or gross can be empty, they can also both be empty, make the two optional
...
\s*?<\/p>)?(\s*?<\/div>\s*?<\/div>\s*?)(<div class="lister-item mode-advanced">|</div>
\s*?<div class="nav">)!is';
...
preg_match_all($regex, $result, $match);
        ...
        for ($i=0;$i<count($match[0]);$i++) {
            if (preg_match('!Votes:</span>\s*?<span name="nv" data-value="(\d*?)">!is',
             $match[0][$i], $votes)) {
                $movies['votes'][$i] = $votes[1];
            } else {
                $movies['votes'][$i] = '';
            }
            if (preg_match('!Gross:</span>\s*?<span name="nv" data-value="(.*?)">!is',
            $match[0][$i], $gross)) {
